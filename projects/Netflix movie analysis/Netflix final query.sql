-- Open netflix dataset in Excel
-- Split the cast column by comma using "Text to column" option in excel
-- Also split director,country,listed_in

-- Create new sheet with common column 'Show_id' 
-- 	for 'Description,netflix_cast,netflix_directors,netflix_listed_in,countries and remaining columns with name netflix_tiles
-- Replace the blanks with Null

-- Group the splitted columns in every table into union using SQL query and save it to new table
-- For cast table
...
--  Import the data into PoerBi to create 'NETFLIX DASHBOARD'
